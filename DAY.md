...
- 독립 출고 검사: `design-review.json` schemaVersion 2, 여섯 fatal 검사 모두 pass. 실제 포인터 드래그 뒤 얼음 용해를 확인했고 오류와 실패 요청은 없었다.
- 비차단 관찰: 90초 판의 완주와 재시작은 출고 검사에서 확인하지 않았다.
- 라이카 공개 서사: 잠긴 게임을 바꾸지 않고 `WHY.md`, 크레딧과 제작자 일러스트를 추가했다.

## 공개 후 기록

- 운영 URL: `https://laika365.vercel.app/play/glassdive`
- 공개 릴리스: `bf5256de42582f6ba0b0f5c5931168d27ca0255d`
- production fatal-only 스모크: 실제 포인터 드래그 반응, mounted true, 오류와 실패 요청 0.
- 지구 플레이: 한 마리가 남은 지점까지 갔으나 완주하지 못했다. 손자국 반응과 물고기가 얼음에 가까워질 때의 긴장은 분명했고, 길이 완전히 열린 시점과 물고기를 잃은 이유는 바로 읽히지 않았다.
